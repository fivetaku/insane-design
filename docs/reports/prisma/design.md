...
Prisma's current homepage has moved away from the older indigo-first identity and now behaves like a managed database control room washed in teal light. The first screen is mostly white, but not empty: it is sprayed with tiny `#14B8A6` (`{colors.primary}`) particle marks and a pale `#F0FDFA` (`{colors.surface-ppg}`) atmospheric wash, so the hero reads as Postgres infrastructure without becoming a literal database diagram. It feels less like a SaaS splash page and more like a clean-room observation window: the database machinery is implied by telemetry specks, not shown as machinery.

The type is the product. The hero phrase "Postgres, perfectly managed." lands in extra-heavy Inter, centered, compressed into two lines, and allowed to dominate the viewport before any product UI appears. It has the weight of a label etched onto a server-room door: short, black, operational, and not trying to entertain. The design does not explain everything at once; it makes one promise, then gives a single teal action and a mono command as the developer-native proof.

The palette is two-track. Prisma's application token system exposes `--primary: #16A394` (`{colors.primary-token}`), but the marketing hero's visible CTA and particle signature use `#14B8A6` (`{colors.primary}`) as the remembered color. There is no second brand color fighting for the stage: indigo exists in the system, but the homepage refuses to become purple Prisma again. Secondary structure stays neutral: `#111827` (`{colors.text-primary}`) text, `#F9FAFB` (`{colors.surface-soft}`) panels, `#E5E7EB` (`{colors.border}`) strokes, and low alpha black shadows. The teal is not sprinkled everywhere as decoration; it marks database creation, selected product affordances, and success/PPG surfaces.

The page is also split between documentation discipline and marketing polish. Fumadocs-style `--color-fd-*` tokens, Tailwind v4 theme variables, and monospace command treatment sit underneath a large SaaS hero. The command chip works like a terminal receipt placed beside the sales counter: the marketing promise is immediately backed by a runnable artifact. That hybrid is the Prisma signature: a serious developer surface with a launch-page rhythm, not a playful startup collage.

The hero's negative space is doing product work. White is not luxury emptiness here; it is the lab bench that makes the teal particles and the command syntax legible. Shadows are similarly restrained: chrome lifts just enough to clarify controls, while the memorable atmosphere belongs to the `#14B8A6` particle field rather than to glossy cards.

비유로 한 번 더 풀자면, prisma.io는 **Postgres 시연이 진행 중인 실험실 작업판**이다. 흰 hero는 라미네이트 lab bench, teal particle field는 클린룸의 telemetry pinpoint, 헤드라인은 작업판 위에 인쇄된 blueprint 도면 라벨이다. teal CTA는 도면 옆에 놓인 시연 시작 스위치, mono command chip은 그 옆 공구함에서 꺼낸 라벨된 종이 receipt — `npx prisma init`은 시연자가 시연 직전에 손에 쥐는 측정 도구다. 아래쪽의 workflow card grid는 엔지니어링다이어그램의 부품 박스, 작은 teal icon cell은 도면 한구석에 찍힌 색인 마크다. 페이지가 마케팅 splash가 아니라, 데이터베이스 인프라의 시연 매뉴얼처럼 정렬되어 있다.
