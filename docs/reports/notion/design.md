...
Notion's current homepage is no longer the quiet blank document page people remember. The captured first viewport is a stage set: a dark midnight plane, a huge centered headline, a glowing product window, and hand-drawn companion icons orbiting the workspace like small signals in a night shift. The product promise is operational, but the surface is theatrical.

The old Notion identity still sits underneath. Its neutral system remains warm, not sterile: `#F6F5F4` (`{colors.surface-neutral}`), `#191918` (`{colors.text-primary}`), `#615D59` (`{colors.text-secondary}`), and soft hairlines from the gray ramp. That warmth matters because it keeps the AI story from becoming a generic blue SaaS page. The public hero can go dark, but the product chrome returns to the familiar paper-and-ink workspace. It is less "AI dashboard" than a desk lamp left on over a shared notebook.

The signature move is the contrast between "night" and "notebook." A deep blue hero field carries the AI narrative, then a white workspace mockup lands in the middle with board columns, side navigation, small agents, task pills, comments, and app badges. The contrast says: AI is active in the background, but work still resolves into readable documents, databases, and tasks. The site behaves like a theater after closing: the room is dark, the crew is still moving, and the only fully lit object is the work table.

Typography is dense and engineered. `NotionInter` is used as the primary sans, with large headings pulled tight through negative tracking tokens. Body text stays calm and legible, while display text gets compressed enough to feel like a product launch headline. `Lyon Text` and `iA Writer Mono` are present as system assets, but the homepage's first impression is sans-led. The display text has ink pressure rather than poster gloss: the letters feel pressed into the midnight field, while the product UI returns to small document labels and database chips.

The page's craft is in restraint plus tiny irregularity. The UI chrome is precise, the shadows are layered and soft, the radius scale is modest, and the illustrated accents are deliberately loose. It does not let the doodles take over. The doodles work because they appear around a disciplined workspace frame. There is no second brand color show; `{colors.accent-blue}` and `{colors.hero-cta}` handle action, while paper neutrals do the actual organizing. Shadow is not used as decorative drama across the page; it belongs to the workspace window, the one object that must feel touchable.
