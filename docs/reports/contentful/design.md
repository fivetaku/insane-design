...
Contentful's current homepage behaves like a B2B SaaS product that has decided not to look like a control panel. The page is light, rounded, and conversational, but the underlying grid is strict: a 12-column desktop scaffold, a 1320px max container, and repeated 1.6rem / 2.4rem / 3.2rem spacing units. The visible personality comes from the contrast between that rigid layout system and the soft mint hero panel.

The hero is a "friendly operating room" for content chaos: not sterile white, but a pale mint work surface where messy campaigns, channels, and AI promises are laid out under calm light. #EFF2F6 (`{colors.mist}`) behaves like frosted glass over a planning table, while #2B2D31 (`{colors.ink}`) arrives as the surgical instrument: H1 ink, CTA fill, and the one dark object that gives the soft scene authority.

The brand blue #1770E5 (`{colors.primary}`) is not wallpaper and not a second sky. Contentful keeps it like the illuminated button on a content orchestration console: dormant until intent, then bright on hover, links, and emphasis moments. There is no full blue wash, no ambient gradient cloud, no "make SaaS = blue" reflex; the page stays white and misted until interaction asks for electricity.

Typography does most of the conversion work. Avenir Next W01 gives the page rounder terminals than Inter, and the headline weight sits at 600 instead of jumping to a heavy 800/900 marketing voice. Body copy remains readable at 16px with a relaxed line-height around 26px. This is an enterprise site speaking in direct sentences, not a developer tool pretending to be documentation.

The site's craft is in its soft boundaries: 2rem buttons, 1.2rem and 2rem cards, rounded hero panels, and almost no chrome shadow. It feels less like a stack of dashboards and more like a content studio with rounded trays: every module has a rim, but very few cast a theatrical shadow. Borders and surface tint define hierarchy more than elevation; shadow is a spare tool for overlays, not the brand voice.

Navigation is the other signature object. The mega menu reads like a library card catalog rebuilt for a DXP: product, solution, resource, developer, partner, and pricing taxonomy in compact Avenir labels, small icons, and muted descriptions. The site does not hide complexity; it shelves it neatly. That is the Contentful move: chaos is not deleted, it is filed into a friendly 12-column archive.
