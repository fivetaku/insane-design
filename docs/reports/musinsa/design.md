...
MUSINSA is not trying to look like a luxury editorial site. It behaves like a fashion trading floor: the top chrome is dark, compact, and operational; the page below is bright, dense, and image-led. The black header at #1A1B1F (`{colors.chrome-dark}`) frames the service as a control surface, while the product and campaign imagery carries the seasonal emotion.
...
The page feels like a black checkout counter attached to a very bright stockroom. The 56px dark navigation is the cashier's rail: narrow, horizontal, and always in charge. Below it, #F5F5F5 (`{colors.surface-section}`) works like a fitting-room floor and #FFFFFF (`{colors.surface-base}`) becomes the clean shelf surface where search, cards, and dialogs sit. Campaign photography is the hanging garment; the interface is the rack hardware.

Spacing is compressed but not sloppy. Navigation, tab rows, product chips, and footer cells use a strict 4px ladder with 13px and 14px body text. The feeling is less "hero page" and more "well-racked store wall": everything is reachable, categorized, and always one click away. Large campaign cards provide the visual pause, then the interface immediately returns to dense commerce. MUSINSA's air is not luxury whitespace; it is aisle width, just enough for scanning.

MUSINSA's craft is in operational consistency: 56px global navigation, 52px appbar, 40px floating buttons, 4px default radius, 1px hairlines, 13/14px list typography, and pill-like counters. The design avoids decorative flourish because every page must absorb new stores, campaigns, ranking states, and editorial content without breaking rhythm.

The blue is deliberately tiny, like a price gun sticker or stock-room LED. #245EFF (`{colors.action-blue}`) appears as a 16px counter or a 4px dot, never as a wall color. Shadows are similarly withheld: chrome does not float, cards do not become SaaS pillows, and depth comes from hairlines plus photography. The site has almost no "website" self-consciousness; it behaves like a retail operating system that keeps clearing space for products, rankings, and campaigns.
