...
Coupang is not a calm editorial storefront. It is a high-throughput market floor with a very disciplined blue control rail. The top chrome is almost pure utility: logo, category block, search, account, cart, shortcut strip. Everything important is reachable before the page asks the user to read a paragraph. The visual anchor is not a hero headline in the SaaS sense, but the search box and category blue, both locked to #346AFF (`{colors.ui-action-blue}`).

The surface is deliberately white and pale gray, like a warehouse floor painted so inventory can move fast without visual friction. #FFFFFF (`{colors.ui-b-color-white}`) carries navigation, cards, right rail modules, and product grids; light gray campaign bands give the hero enough separation without turning the site into a branded canvas. Coupang's real brand is therefore split: the logo owns the multi-color identity, while the interface delegates action to blue and urgency to red #CB1400 (`{colors.ui-commerce-red}`). There is no second brand color in the shell. Red is not a sibling to blue; it is the price tag, coupon stamp, and warning tape.

Typography is compact Korean commerce typography. The page spends most of its type budget on 11-14px labels, category names, prices, and utility links, like shelf labels in a very large store; then it suddenly jumps to very large 700-weight campaign typography inside ad creatives. This contrast is important: the platform UI stays small so the promotion artwork can shout. The shell behaves like logistics software, while the banners behave like supermarket flyers taped over the aisle.

The search bar is the cashier belt and conveyor gate at once: a rectangular blue-edged machine that turns vague intent into SKU flow. The blue category slab is not a button pretending to be friendly; it is a control-room switch, flat and vertical, interrupting the white header with one hard operational command. Shadow is similarly rationed. It belongs to dropdowns and floating rails, not to every product card; the marketplace floor is separated by hairlines, not by decorative elevation.

The design metaphor is a logistics console overlaid with supermarket flyers. The console part is rectangular, gridded, and reliable; the flyer part is image-led, seasonal, and loud. Product photography and campaign images carry the rotating color weather, while the chrome refuses to become a mood board. Rebuilding Coupang means preserving that tension rather than polishing it into a minimalist app.

To stretch the metaphor further: the right-rail modules act like the pickup counter beside the checkout aisle, the breadcrumb shortcut strip behaves like the colored floor tape that guides forklifts through a fulfillment center, and the rocket badge lands like a stamped bill of lading on each delivery-eligible product. The category column is a row of warehouse aisle markers, not a navigation menu, and the search box behaves like the barcode scanner gate at the loading dock — tap intent in, SKU stream out. There is no second brand color for the chrome because a warehouse floor does not need wallpaper.

조금 더 풀어보면, 쿠팡 홈은 **24시간 불 켜진 물류센터의 카운터 앞**처럼 작동한다. 검색바는 입고된 SKU를 스캔하는 카운터이고, 파란 카테고리 슬랩은 창고 통로 앞에 세워진 색상 안내판이다. 상품 그리드는 매대 위에 줄지어 깔린 진열대 묶음, 우측 레일은 픽업 카운터 옆 빠른-결제 라인, 그리고 광고 배너는 시장 입구에 붙은 시즌 카탈로그 포스터다. 장바구니 아이콘은 마켓의 회수 카트처럼 고정된 자리에서 절대 사라지지 않는다 — 두 번째 브랜드 컬러가 없는 이유는, 물류센터 바닥에 컬러 wallpaper를 깔 자리가 없기 때문이다.
