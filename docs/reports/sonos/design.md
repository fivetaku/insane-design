...
Sonos builds the page like a listening room where the walls have been painted out of the photograph. The interface is not the room; it is the quiet white shelf at the edge of the room. A 64px retail nav, black wordmark, black icons, and a thin #E0E0E0 (`{colors.hairline}`) seam hold the commerce layer in place while the emotional volume moves into full-bleed domestic media and oversized white type.

The homepage feels less like an app shell than a product film paused on the cleanest frame: sofa, speaker, dim room, sound implied but never illustrated. The hero is a small home theater screen with the lights down. White display type does not sit in a card; it floats directly over the scene, as if the product claim is part of the room's acoustics. There is no visible caption panel, no decorative gradient stage, no second brand color trying to become the bass line.

The dominant color system is almost binary: #FFFFFF (`{colors.surface-white}`) for retail clarity, #000000 (`{colors.brand-black}`) for authority, and #2E2E2E (`{colors.text-charcoal}`) for dense product copy. The few chromatic tokens, such as #DE8579 (`{colors.accent-red}`) and #C59C6E (`{colors.accent-gold}`), behave like finish samples on a speaker grille or campaign lighting in a living room. They are not the interface's everyday voice.

The signature move is the contrast between ordinary navigation and cinematic hero. The white nav is a showroom counter: practical, flat, easy to scan. Under it, the page drops into a darker sound stage, then a huge centered headline cuts through the frame in #FFFFFF (`{colors.surface-white}`). The CTA pair is a black/white volume knob: one white pill for the named product, one black pill for the broader catalog. Commerce hierarchy happens through inversion, not color decoration.

Typography is utilitarian but big. `aktiv-grotesk` carries both display and body, which means Sonos relies on acoustic scale, weight, and scene composition rather than font contrast. Display settings are not tightly tracked; the hero headline keeps a broad consumer-electronics voice, more living-room confidence than luxury compression. The page has almost no self-conscious site chrome: photography is the speaker cabinet, black/white pills are the controls, and everything else steps back.
