...
NAVER is not a landing page. It is a civic dashboard disguised as a homepage: search is the ceremonial center, but the surrounding surface is a dense utility grid. The page opens like the front desk of a very busy public building: the broad white field gives you one calm counter, then the green search pill tells every visitor where to speak first. That single outline and icon color, #03C75A (`{colors.primary}`), does nearly all brand work while the rest of the page stays procedural.

The site's craft is in compression. Shortcuts sit as small square icons, cards have thin hairlines, and news/shop/widget modules stack without dramatic section breaks. It is closer to a newspaper kiosk wired into a control panel than to a magazine cover: many doors are visible at once, but the signage is disciplined. It avoids editorial spectacle because the product is not one story; it is a daily command surface. Every module is allowed to be visible at once, but almost none is allowed to shout louder than search.

The palette is mostly administrative: #FFFFFF (`{colors.surface}`), #F0F0F0 (`{colors.page}`), #101010 (`{colors.text}`), and gray captions around #8C8C8C (`{colors.text-muted}`). Chromatic color appears in service identity chips and topic filters, not as page atmosphere. News blue #0147B5 (`{colors.link-blue}`), entertainment magenta #E65DA0, shopping yellow #EBAA00, and webtoon green #00C855 behave like subway line colors on a neutral station map. There is no second global brand color; blue, yellow, red, and magenta are route labels, not mood lighting.

Typography follows Korean portal density: system UI fonts, Malgun Gothic compatibility, and a lot of medium/bold weights. The search placeholder is intentionally large and pale, like a blank form field waiting at the counter; news and widget labels are smaller, darker, and tightly tracked. The page does not ask to be admired as a composition. It behaves like a switchboard: white cards are panels, hairlines are seams, and the green command ring is the one lit button you cannot miss.

NAVER's negative identity matters as much as its visible parts. No cinematic hero, no full-bleed photography, no manifesto headline, no luxury whitespace. The page has almost no "site" self-consciousness; it gets out of the way by becoming infrastructure. Its atmosphere is not minimalism, but regulated abundance: a portal city block where every storefront is small, labeled, and open.

이 페이지는 한 도시의 **공공 안내 데스크**다. 검색 pill은 모든 시민이 한 줄로 서서 말을 거는 단일 카운터, 그 아래 shortcut rail은 데스크 옆 안내 표지판, news/widget cards는 안내문이 꽂힌 게시판이다. 어떤 광고도 카탈로그도 도시의 매대를 통째로 차지하지 않는다 — 색은 지하철 노선처럼만 쓰이고, 카운터의 형광등은 단 하나의 #03C75A 링이다. 매거진의 표지가 아니라, 매일 같은 시각에 켜지는 시민 정보 데스크의 LED 표지등이고, 데스크 양옆의 게시판은 짧은 라벨로만 정렬된다. 도시는 침묵하지 않고, 하지만 외치지도 않는다.
