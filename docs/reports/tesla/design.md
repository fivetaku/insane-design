...
Tesla's homepage is not built as a colored interface. It is a glass showroom with the walls removed: white nav, centered model name, two pill CTAs, and full-bleed vehicle media do just enough work to orient the visitor, then vanish into the product surface. The brand is not a badge treatment or a wallpaper color. It is sky reflected on glass, wheels planted against asphalt, red paint held inside photography, and a horizon line acting like the page grid.

The page's main act is the viewport-sized automotive hero. The fetched screenshot shows a white fixed top navigation, then a giant media field where `Model Y` sits in white over a pale sky, with `Order Now` in #3E6AE1 (`{colors.primary}`) and `View Inventory` in #FFFFFF (`{colors.surface-white}`). It feels less like a web banner than a delivery bay door opening: the interface is the floor tape, the car is the event. Tesla makes the chrome deliberately ordinary so the object can look inevitable.

The palette is a negative-space palette. #F4F4F4 (`{colors.surface-light}`) and #FFFFFF (`{colors.surface-white}`) are the showroom concrete and glass. #171A20 (`{colors.text-primary}`) is not theatrical black; it is the printed ink on a specification placard. #3E6AE1 (`{colors.primary}`) is the cashier's blue, not a brand mural. There is no second UI brand color: red belongs to paint, tail lights, and assets, not to the button system.

Typography follows the same industrial restraint. The model name reads like badging on the rear of a vehicle: short, centered, mechanically clean, and confident because it has almost no sentence around it. Navigation labels behave like labels above cars in a showroom, small enough to disappear until needed. Tesla avoids editorial serif contrast, playful weights, and expressive type detail because any flourish would compete with the product photography.

The craft is in controlled staging: full viewport sections, dark or light scrims, carousel dots, small square arrow controls, and CTA groups that stay stable while media changes underneath. Shadow is not a chrome language here; depth comes from the photograph. The site has almost no self-consciousness as a site. It behaves like an automated showroom turntable where the page machinery keeps moving, but the visitor remembers the vehicle, the sky, and the two decisions waiting below the headline.
