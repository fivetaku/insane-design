...
BCG의 웹 표면은 컨설팅 회사의 "지적 권위"를 과시하는 방식이 아니라, 편집국과 전략 워룸이 섞인 방식으로 작동한다. 첫 화면의 언어는 "Unlocking the Potential of Those Who Advance the World"처럼 큰 선언이지만, 시각적 장치는 과장되지 않는다. 대신 #212427 (`{colors.text-primary}`)에 가까운 차콜 텍스트, #FFFFFF (`{colors.surface-base}`) 기반의 뉴스룸식 표면, 그리고 #7EF473 (`{colors.accent-primary}`) CTA가 정확히 필요한 순간만 들어온다. 사이트는 컨설팅 로비의 대리석 벽보다, 새벽에 아직 불이 켜진 리서치 편집국에 가깝다. 종이는 흰색이고, 문장은 차분하며, 행동해야 할 지점에만 형광펜 같은 라임이 한 줄 그어진다.

가장 기억되는 장면은 BCG의 딥그린보다 라임에 가깝다. BCG는 전통적인 그린 브랜드처럼 모든 것을 녹색으로 칠하지 않는다. no second brand color: #7EF473 (`{colors.accent-primary}`)가 primary action의 단일 신호이고, #0E3E1B (`{colors.green-deep}`)는 CTA가 아니라 구독/강조 밴드의 기관적 무게를 만든다. 딥그린은 보드룸의 벽처럼 뒤에 서 있고, 라임은 회의 테이블 위에 놓인 결정 버튼처럼 앞으로 나온다. 이 둘 사이에 #F1EEEA (`{colors.surface-warm}`)와 #DCD5CE (`{colors.hairline}`) 같은 따뜻한 뉴트럴이 깔려, 금융권 사이트의 차가운 흰색이 아니라 종이와 리서치 리포트에 가까운 질감을 만든다.

타이포그래피는 Henderson BCG Sans가 주도한다. 히어로와 큰 제목은 300 weight로 넓게 호흡하고, 카드 제목과 본문은 400에 머문다. 반면 작은 버튼/라벨은 12px, 700, uppercase로 급격히 조여진다. 이 대비가 BCG의 표정이다. 큰 생각은 얇고 넓게, 실행 액션은 작고 단단하게. 마치 전략 보고서의 표지는 얇은 활자로 넓게 열리고, 결재 도장은 작지만 짙게 찍히는 운용이다. weight 500이 빠져 있는 것도 우연이 아니다. 중간 회색의 타협보다, 생각과 행동을 분명히 나누는 편집 판단에 가깝다.

레이아웃은 콘텐츠 매거진이지만 컴포넌트는 엔터프라이즈 앱처럼 상태가 촘촘하다. primary button에는 default, hover, focus, active, disabled 상태가 변수로 분리되어 있고, hover 때 좌측 padding이 55px까지 확장되며 화살표가 움직인다. 버튼은 단순한 색 면이 아니라, 문장 옆에 열리는 작은 통로다. 화살표가 지나갈 공간을 먼저 만들고, 그 다음 행동을 권한다.

고정 헤더는 pill형 유리막처럼 `backdrop-filter: blur(20px)`를 쓰며, 컨설팅 브랜드에 흔치 않은 물성감을 만든다. 평평한 기업 사이트의 흰색 바가 아니라, 전략 워룸 유리문에 붙은 반투명 명패처럼 페이지 위에 떠 있다. mega menu가 열리면 이 유리 명패는 사라지고 #FFFFFF (`{colors.surface-base}`)의 전체 표면으로 바뀐다. shadow는 모든 카드에 뿌려지는 장식이 아니라 header/search 같은 overlay chrome에 제한되고, cards는 photography와 hierarchy가 먼저 말하게 둔다.

이 페이지는 컨설팅 회사의 보드룸이 아니라 **전략 매거진의 편집국**처럼 운영된다. fixed header pill은 잡지 표지의 마스트헤드, 96px H1은 매거진 1면의 사설 제목, 하단의 article cards는 편집자가 골라 놓은 칼럼 인덱스다. lime CTA는 광고 페이지가 아니라 칼럼 끝의 형광펜 표시이고, deep green band는 정기 구독 페이지를 위한 사설 박스다. BCG의 표면은 카탈로그가 아니라 매거진의 인쇄면 — 광택 종이가 아니라 무광 리포트 페이지에 가깝다. 스토리북의 호흡으로 섹션을 넘기되, 각 섹션은 편집자의 선택으로 정렬되고, 사이트의 톤은 잡지 표지가 아니라 잡지 본문의 활자 리듬이다.
