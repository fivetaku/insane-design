...
Discord's current homepage does not behave like a restrained SaaS page. It opens like a midnight LAN-party room where the lights have been turned off and the monitors are doing the architecture: a very dark night-sky gradient, a large angled app mockup, mascot-like characters, floating doodles, and blunt white type. The first impression is theatrical, but the system underneath is more disciplined than the illustrations suggest.

The key color is still blurple, but it is not sprayed everywhere. There is no second equal brand color. The hero lets the atmosphere move from near-black into saturated blue-violet, then reserves #5865F2 (`{colors.brand}`) for the browser CTA and system states. White is the real workhorse: logo, nav, headline, body copy, and the download CTA all sit on the same dark field. The white CTA is almost a lit keyboard key on the desk; the blurple CTA is the only glowing system button. This is why a Discord-like page fails quickly when the background becomes plain #FFFFFF or when blurple becomes a generic gradient wash.
...
The layout metaphor is "chat room as arcade cabinet." The product UI is visible as a real dark app surface, but it is embedded among props: controller, characters, device mockups, stickers, soft stars, and floating objects. Shadow is not the hero's depth engine; the room is built from layered illustration, tilted screens, and the not-quite-black app shell #23272A (`{colors.not-quite-black}`). The mockup behaves like a broadcast overlay from a friends-only stream rather than a polished enterprise screenshot.

The page sells belonging and play before it sells features. It has almost no "site as brochure" self-consciousness: the page tries to disappear into a shared room, then lets the Discord app, characters, and hard white poster type act as the furniture. The technical contract is therefore simple: build one immersive dark stage, put heavy white type on the left, place product imagery and characters on the right, and let rounded pill CTAs sit below as the conversion controls.
