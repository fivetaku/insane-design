...
> 상대 경로(`real/`, `designmd-data/`)만 쓰면 cwd에 따라 이중 경로가 생긴다.
...
```bash
mkdir -p "$WORK_DIR/real/{slug}/screenshots"
mkdir -p "$WORK_DIR/designmd-data/real/{slug}/{css,phase1}"
```
...
curl -sL \
  ...
  -o "$WORK_DIR/designmd-data/real/{slug}/index.html" \
  "{url}"
```
...
```bash
grep -oE 'href="[^"]+\.css[^"]*"' "$WORK_DIR/designmd-data/real/{slug}/index.html" | \
  sed 's/href="//;s/"$//' | \
  xargs -n1 -P8 -I{} curl -sL -o "$WORK_DIR/designmd-data/real/{slug}/css/$(basename {})" "{absolute_url}"
```
...
curl -sL -H "X-Respond-With: screenshot" --max-time 30 \
  "https://r.jina.ai/{url}" \
  -o "$WORK_DIR/real/{slug}/screenshots/jina-hero.png"

# PIL crop (1280×1280 → 1280×800)
python3 -c "
from PIL import Image
img = Image.open('$WORK_DIR/real/{slug}/screenshots/jina-hero.png')
w, h = img.size
cropped = img.crop((0, 0, w, min(800, h)))
cropped.save('$WORK_DIR/real/{slug}/screenshots/hero-cropped.png')
"
```
...
**반드시 `designmd-data/` 디렉토리에서 실행** (스크립트가 `real/{slug}/` 상대 경로를 사용하므로):
...
결과: `$WORK_DIR/designmd-data/real/{slug}/phase1/` 에 4개 JSON:
...
1. Read: `real/{slug}/screenshots/hero-cropped.png` (또는 `jina-hero.png`)
2. Read: `designmd-data/real/{slug}/phase1/brand_candidates.json`
3. Read: `designmd-data/real/{slug}/phase1/typography.json`
4. Read: `designmd-data/real/{slug}/phase1/alias_layer.json`
...
3. Read: `real/{slug}/design.md` — 데이터 소스
4. Stripe report의 `<style>` 블록 + JS 구조를 기반으로, 토큰/섹션만 현재 서비스로 교체
5. Write: `real/{slug}/report.ko.html`
...
head -1 real/{slug}/design.md | grep -q '^---$'

# 2. 필수 섹션 존재
for n in 01 02 03 04 05 06 07 08 12 14 16; do
  grep -q "^## $n\." real/{slug}/design.md || echo "Missing §$n"
done

# 3. 파일 크기
[ $(wc -c < real/{slug}/design.md) -ge 8000 ]
[ $(wc -c < real/{slug}/report.ko.html) -ge 20000 ]

# 4. hex 실존 (상위 3개)
for hex in $(grep -oE '#[0-9A-Fa-f]{6}' real/{slug}/design.md | sort -u | head -3); do
...
📄 design.md:     real/{slug}/design.md ({size}KB)
🌐 report.ko.html: real/{slug}/report.ko.html ({size}KB)
📸 screenshot:     real/{slug}/screenshots/hero-cropped.png
...
| 출력 경로 | `real/{slug}/` | Step 1에서 변경 가능 |
